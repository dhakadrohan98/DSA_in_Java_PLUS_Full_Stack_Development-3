...
int[] b = new int[n];
while(j<=n) {
	int b[j] = new Scanner(System.in).nextInt();
	i++;
}
...
while(!q.empty()){
    ...
    for(auto children : G[parent]){
        ...
        // used[children]==0 means it has never been reached before.
                ...
                if(b[children]==1) {
                	answer[children] = answer[parent] + 1;
                }
                else {
                	//value of answer[children] will remain same.
                	answer[children] = answer[parent];
                }
